...
export function queryPostgresErrorRate(environment: string): string {
  return `max(last_5m):avg:trace.pg.query.errors{env:${environment}} by {service}.as_count() / avg:trace.pg.query.hits{env:${environment}} by {service}.as_count() > 0.8`;
}
...
export function queryHttpErrorRate(environment: string): string {
  return `max(last_5m):avg:trace.http.request.errors{env:${environment}} by {service}.as_count() / avg:trace.http.request.hits{env:${environment}} by {service}.as_count() > 2.5`;
}
...
export function queryHttpResponseTime(environment: string): string {
  return `max(last_5m):max:trace.http.request.duration{env:${environment}} by {service} > 5`;
}
...
export function queryExpressErrorRate(environment: string): string {
  return `max(last_5m):avg:trace.express.request.errors{env:${environment}} by {service}.as_count() / avg:trace.express.request.hits{env:dev} by {service}.as_count() > 1.5`;
}
...
export function queryExpressResponseTime(environment: string): string {
  return `max(last_5m):avg:trace.express.request.duration{env:${environment}} by {service} > 2`;
}
